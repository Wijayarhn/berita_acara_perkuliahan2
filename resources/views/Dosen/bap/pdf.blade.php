...
<body>
  ...
  <table>
    ...
    <tr>
      ...
      <td>Lokasi</td>
      <td>
        {{ $bap->lokasi_dosen_nama ?? ($bap->lokasi_dosen ?: '-') }}
        @if (!empty($bap->lokasi_dosen_nama) && !empty($bap->lokasi_dosen))
          <br><small style="color:#666;">Koordinat: {{ $bap->lokasi_dosen }}</small>
        @endif
      </td>
    </tr>
    <tr>
  ...
  <table style="width: 100%; border: none; margin-bottom: 5px;">
    <tr>
      ...
      <td style="width: 45%; text-align: center; vertical-align: top; border: none;">
        @php
          $ttdMahasiswa = $bap->bapMahasiswa->filter(fn($bm) => $bm->ttd_mahasiswa)->values(); // pastikan reindex
        @endphp

        @for ($i = 0; $i < 2; $i++)
          @php
            $bm = $ttdMahasiswa->get($i);
          @endphp

          <div style="margin-bottom: 20px;">
            <strong>Mahasiswa {{ $i + 1 }}: {{ $bm->mahasiswa->nama ?? '-' }}</strong><br>
            NIM: {{ $bm->mahasiswa->nim ?? '-' }}<br>
            @if ($bm && $bm->ttd_mahasiswa)
              @php
              ...
              @endphp
              <img src="{{ $base64 }}" alt="TTD Mahasiswa {{ $i + 1 }}" width="100"
  ...
  <div style="text-align: center; margin-top: 5px;">
    ...
    <div style="height: 50px;"></div>
    @if ($bap->verifikasiAdmin)
      <strong>{{ $bap->verifikasiAdmin->nama }}</strong><br>
      (TU Fakultas)
    @else
